...
EMA_TOOL_DEFINITIONS: list[dict[str, Any]] = [
    ...
    {
        "type": "function",
        "name": "check_visit_lapse",
        "description": (
            "Check whether patient had a completed visit within ~12 months. "
            "Use before Rx refill. If lapsed/no history → refuse remote refill and offer book. "
            "READ ONLY. Never e-prescribe."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "window_days": {
                    "type": "integer",
                    "description": "Override lapse window days (default ~365)",
                },
            },
            "required": ["patient_id"],
        },
    },
    {
        ...
        "description": (
            ...
            "Runs ~12mo visit lapse check first; if lapsed returns offer_book. "
    ...
    },
    {
        ...
        "description": (
            ...
            "NOT a prescription. Queues inventory/staff message. Distinct from request_rx_refill. "
            "Requires confirmed=true + EMA_WRITES_ENABLED to queue."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
def handle_ema_tool(name: str, arguments: dict) -> str:
    ...
    try:
        ...
        elif name == "check_visit_lapse":
            pid = arguments.get("patient_id")
            if pid is None:
                return _compact_json({"error": "patient_id_required"})
            refill = _get_refill_flow()
            wd = arguments.get("window_days")
            result = refill.check_visit_lapse(
                pid,
