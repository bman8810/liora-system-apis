...
class GrokBridge(AIBridge):
    ...
    async def configure_session(self, patient_name: str = "the patient", dial_phone: str = ""):
        ...
        if self.enable_ema_tools:
            instructions = config.SYSTEM_INSTRUCTIONS_SCHEDULING.format(
                ...
                dial_phone=dial_phone or "the number we called",
            )
        else:
    ...
    async def _handle_function_call(self, event: dict):
        ...
        logger.info("Grok function call: %s call_id=%s args_keys=%s has_dob=%s has_phone=%s has_last=%s", name, call_id, list(arguments.keys()), bool(arguments.get("dob")), bool(arguments.get("phone")), bool(arguments.get("last_name")))
